...
public class EnemyManager : MonoBehaviour
{
    ...
    #region NEW VERSION
    public void Bullet(int offset)
    {
        spawnTimer += Time.deltaTime;
        if (spawnTimer >= spawnRate)
        {
            Instantiate(EnemyWeapon, new Vector3(transform.position.x - offset, transform.position.y, transform.position.z), Quaternion.identity);
            spawnTimer = 0;
        }
    }
    #endregion
